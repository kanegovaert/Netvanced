...
namespace Netvanced_Proxy_Tool
{
    ...
    public partial class NetvancedForm : Form
    {
        ...
        {
            if (TB_ProxiesToCheck.Text != null)
            {
                int ProxyCount = LV_ProxyChecking.Items.Count;

                foreach (string Proxy in TB_ProxiesToCheck.Lines)
                {
                    ListViewItem ProxyCountItem = new ListViewItem();
                    ProxyCount++;
                    ProxyCountItem.Text = ProxyCount.ToString();
                    LV_ProxyChecking.Items.Add(ProxyCountItem);
                    ProxyCountItem.SubItems.Add(Proxy);
                }
            } 
        }

        private void BTN_ClearCheckedProxies_Click(object sender, EventArgs e)
        {
            LV_ProxyChecking.Items.Clear();
        }

        private void BTN_SaveProxies_Click(object sender, EventArgs e)
        {
        }

        private void BTN_CheckProxies_Click(object sender, EventArgs e)
        {
        ...
        {
            System.Diagnostics.Process.Start("https://wwww.github.com/kanegovaert");
        }
